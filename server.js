	...
	const MONGO_DATABASE_NAME   = 'p2pbettingplatformdb_bugfix';
	const MONGO_COLLECTION_NAME = 'sportscollection_bugfix';
	...
	*/
	...
	io.on('connection', async (socket) => {
		...
		// when client closed/disconnect from the server
		socket.on('disconnect', function() {

			console.log('A Client has closed / disconnected from the Server !');
			console.log('Remaining no. of active Clients connected to the Server : #', g_Storage.nClientsConnected ? --g_Storage.nClientsConnected : 0);
		});
